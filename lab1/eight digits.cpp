...
返回 H
*/
int CountH(int *status,int FinalStatus[9])
{
	int H = 0;
	int i;
	for (i = 0; i <= 8; i++)
...
返回是否存在于OPEN或CLOSE的值，值为0 均不在，值>0 只在OPEN表，值<0 只在CLOSE表，|值|-1表示所在列表中的位置
*/
int Exist(ENode *N)
{
	int i, j;
...
ENode *ENodeInit(int *status, int zero, int g, ENode *parent, int step)
{
	...
	Node->Zero = zero;
	Node->G = g;
	Node->H = CountH(Node->status,FinalStatus);
...
ENode *Astar()
{
	...
	while (1)                   //一直循环知道找到解结束
	{
		...
		*Node = OPEN[0];                 //从OPEN表中取出第一个元素（F值最小）进行扩展
		...
		if (!CountH(Node->status,FinalStatus))      //判断该节点是否是目标节点，若是，则不在位的将牌数为0，算法结束
		{
			break;
		}
...
int main()
{
    ...
    if(!(Revrse(fstatus)%2 == Revrse(FinalStatus)%2)){
        cout << "不可达\n";
        return 0;
    }
